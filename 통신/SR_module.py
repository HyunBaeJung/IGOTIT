...
def main(list):
    ...
    f.write("Buf_"+list[0]+",test\n")#두번째 줄에 test.wav,test라고 쓴다.
    f.close()#파일을 닫는다
    ...
    fbank = {}
    mfccs = {}
    wav_file = df[df.label == 'test'].iloc[0,0]
    signal, rate = librosa.load('test/'+wav_file, sr =None)
    ...
    signals_abs=abs(signals['test']).mean()
    signals_std=signals['test'].std()
    fft_maximum=fft['test']

    if signals_abs>0.1: #데시벨로 2차 거르기
        if signals_std>0.1:
...
#-------------------------------------------------------------------------------------------------
    ...
    # signals_abs=[]
    # signals_abs.append(abs(signals['test']))
    # a=[]
    # for j in range(len(signals_abs['test'])-1):
    #     a.append(abs(signals_abs['test'][j]-signals[i][j+1]))
    # decibel_difference.append(a)
    # decibel_difference_list=[sum(i)/len(i) for i in decibel_difference]
    # if pd.Series(decibel_difference_list).mean()<=0.189218:

    # mean=0.106547, std=0.027557, 99.7프로 말하는소리
